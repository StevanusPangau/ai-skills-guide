...
### Added
- **Official Enterprise & Full-Stack Collections (9 new collections, 120+ skills)**:
  - **Vercel Agent Skills (9 skills)** — `vercel-labs/agent-skills` (MIT): React/Next.js performance rules, composition patterns, React Native, view transitions, deployment, and cost audits. Includes interactive `VercelWaterfallLab` (`/vercel#waterfall-lab`).
  - **Anthropic Agent Skills (19 skills)** — `anthropics/skills` (Copyright Anthropic PBC): Document creation (`docx`, `pdf`, `pptx`, `xlsx`), design & brand (`frontend-design`, `canvas-design`, `brand-guidelines`, `theme-factory`, `algorithmic-art`), agent development (`skill-creator`, `mcp-builder`, `claude-api`, `webapp-testing`, `web-artifacts-builder`), and communication. Includes interactive `AnthropicEvalArena` (`/anthropic#eval-arena`).
  - **Cloudflare Edge Skills (13 skills)** — `cloudflare/skills` (Apache-2.0): Workers platform, Durable Objects, Agents SDK, Sandbox v1 (@next) & v2, Cloudflare One, Turnstile, and Email Service. Includes interactive `CloudflareEdgeMatrix` (`/cloudflare#edge-matrix`).
  - **Supabase Postgres Skills (2 skills)** — `supabase/agent-skills` (MIT): Comprehensive Postgres performance optimization across 8 impact-ranked tiers, multi-tenant RLS subquery caching, connection pooling limits, and platform operations (@supabase/ssr, Data API grants, SECURITY DEFINER search_path). Includes interactive `SupabaseRlsLab` (`/supabase#rls-lab`).
  - **Prisma ORM v7 Skills (9 skills)** — `prisma/skills` (MIT): Prisma ORM v7 architecture migration, mandatory SQL driver adapters, type-safe query API, CLI tooling, serverless Prisma Postgres, containerized Prisma Compute deployment, and strategic terminal v6 guidance for MongoDB. Includes interactive `PrismaV7Lab` (`/prisma#v7-lab`).
  - **TanStack Framework Skills (14 skills)** — `tanstack-skills/tanstack-skills` & TanStack Intent (MIT): Type-safe routing, Query data fetching, headless Table & Form, full-stack Start, Virtual, Store, and AI SDK, with TanStack Intent npm skills distribution. Includes interactive `TanStackLab` (`/tanstack#reactive-lab`).
  - **Expo & EAS Skills (25 skills)** — `expo/skills` (MIT): Universal mobile iOS/Android engineering, Expo Router file-based navigation, `@expo/ui` true SwiftUI & Jetpack Compose native widgets, continuous native prebuild, Reanimated 60FPS worklets, EAS Build cloud compilation, EAS Submit to stores, instant OTA EAS Update hotfixes, and EAS Hosting. Includes interactive `ExpoNativeLab` (`/expo#native-lab`).
  - **GreenSock / GSAP Skills (8 skills)** — `greensock/gsap-skills` (MIT): Industry-standard 60FPS animation engine, ScrollTrigger viewport scrubbing and pinning, timeline choreography with position parameters (`"<"`, `">"`), official `useGSAP` React hook, and GPU-accelerated Flip transitions. Includes interactive `GsapTimelineLab` (`/gsap#gsap-lab`).
  - **Impeccable Design System Skills (21 skills)** — `pbakaus/impeccable` (Apache-2.0): Senior design engineering suite by Paul Bakaus (creator of jQuery UI / AMP), liberating AI-generated interfaces from generic visual slop via `PRODUCT.md` & `DESIGN.md` dual specs (Google Stitch), 4px/8px vertical rhythm normalization, WCAG AAA color contrast audits, and humanized microcopy. Includes interactive `ImpeccableLab` (`/impeccable#impeccable-lab`).

- **Interactive Provider Labs & Simulators (9 dedicated labs)**:
  - **Vercel**: Performance & Waterfall Lab simulating sequential waterfall latency vs `Promise.all` vs deferred branch fetch, boolean prop state explosion (2^N) vs compound components, and the 5-stage 14-day `signals.json` optimization gate.
  - **Anthropic**: Evaluation & Context Arena simulating 3-tier progressive disclosure context window savings (88% token cut), `skill-creator` baseline-vs-skill eval benchmarks (`evals.json`), and deterministic Python scripts vs LLM XML guesswork.
  - **Cloudflare**: Runtime & Binding Matrix with an interactive primitive decision engine (DO vs Worker vs Agents SDK vs Sandbox), in-memory direct bindings vs external REST latency benchmark (1.4ms vs 118ms), and stable vs @next Sandbox package dichotomy.
  - **Supabase**: RLS & Query Lab simulating cached subquery `((select auth.uid()))` cutting 97% scan time, keyset cursor pagination vs OFFSET, and connection pooler RAM budget calculator.
  - **Prisma**: Engine v7 Lab comparing v6 C++ binary vs v7 JavaScript driver adapter architecture, database provider branching, and container 0.0.0.0 binding rules.
  - **TanStack**: Reactive State & SWR Lab demonstrating bidirectional URL search params sync, SWR query cache freshness lifecycle, and fine-grained form re-render isolation.
  - **Expo**: Native UI & OTA Updates Lab demonstrating Safe Area Insets against Dynamic Island / camera notch clipping, and the OTA JavaScript vs native store binary compilation decision tree.
  - **GSAP**: Timeline Choreography & 60FPS Lab featuring interactive ScrollTrigger scrubber, relative timing syntax (`"<0.2"`) vs brittle manual delays, and GPU Composite vs Layout Reflow browser pipeline cost benchmark.
  - **Impeccable**: Anti-AI Slop & Craft Polish Lab comparing bland AI-generated gray cards against bold Impeccable craft, 4px/8px modular rhythm visualizer, and P0-P3 structured audit severity rubric.

- **Deep Visual Architecture Concepts**: Dedicated mental model diagrams and principle cards for all 9 new providers (impact-ranked rule priority, edge compute hierarchy, universal app matrix, continuous native prebuild, unified requestAnimationFrame ticker, dual-spec architecture, and craft floor invariants).
- **Common Workflows**: 3 concrete end-to-end scenarios per collection formatted in Matt Pocock's practical style with technical rationale and executable CLI steps.
- **Per-Skill Spotlights**: Unique, non-templated technical callout cards for all skills highlighting critical nuances, edge cases, and upstream operational caveats.
- **Dedicated Skill Detail Routes**: 120+ new individual deep-link routes (`/$collection/skills/$skillName`) complete with breadcrumbs, badges, install blocks, and "Copy as Agent Prompt" button.
- **Official Brand Avatars**: Added self-hosted Simple Icons SVG avatars in `public/avatars/` (`vercel.svg`, `anthropic.svg`, `cloudflare.svg`, `supabase.svg`, `prisma.svg`, `tanstack.svg`, `expo.svg`, `gsap.svg`, `impeccable.svg`).
- **Global Command Palette Indexing**: Search index expanded across all 15 collections and 220+ skills (`all-skills-search.ts`).
- **Comprehensive Bilingual Localization**: Expanded Paraglide i18n keys to 647 keys in both `messages/id.json` and `messages/en.json` with 100% key parity.
- **Expanded SEO Artifacts**: Generated canonical routes for 238 pages in `public/sitemap.xml` and `public/llms.txt`.

### Fixed
- **Tier 1 Audit Pass**: Official logos for Vercel/Anthropic/Cloudflare across heroes, hub cards, and skill detail pages; removed forced flow diagrams from Anthropic and Cloudflare (kept Vercel's with token-auth node, parallel-review gate, and loop label); corrected 11 factual nuances (rule counts, priority labels, version claims, dry-run limits, cutover approval).
