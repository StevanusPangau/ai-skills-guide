...
### Added
- **Official Expo Skills collection (25 skills)** — `expo/skills` (MIT): complete universal mobile engineering for iOS and Android across Expo Router, native SwiftUI/Jetpack Compose widgets (@expo/ui), development clients, Reanimated animations, and EAS cloud automation (EAS Build, EAS Submit to App Store/Google Play, instant OTA EAS Update, and EAS Hosting). Includes interactive `ExpoNativeLab` (`/expo#native-lab`).
- **Official GSAP Skills collection (8 skills)** — `greensock/gsap-skills` (MIT): industry-standard 60FPS animation engine, ScrollTrigger viewport scrubbing and pinning, timeline sequencing choreography, useGSAP React integration hook, and Flip layout transitions. Includes interactive `GsapTimelineLab` (`/gsap#gsap-lab`).
- **Impeccable Design System collection (21 skills)** — `pbakaus/impeccable` (Apache-2.0): senior design engineering suite by Paul Bakaus (creator of jQuery UI / AMP), liberating AI-generated interfaces from generic visual slop via PRODUCT.md/DESIGN.md dual specs, 4px/8px vertical rhythm normalization, WCAG AAA color contrast audits, and humanized microcopy. Includes interactive `ImpeccableLab` (`/impeccable#impeccable-lab`).
- **Official Supabase Skills collection (2 skills)** — `supabase/agent-skills` (MIT): comprehensive Postgres performance optimization (8 impact-ranked rule tiers: Query, Connection, RLS, Schema, Concurrency, Data Ops, Monitoring), multi-tenant RLS subquery caching, connection pooling limits, and platform operations (@supabase/ssr, Data API grants, SECURITY DEFINER search_path). Includes interactive `SupabaseRlsLab` (`/supabase#rls-lab`).
- **Official Prisma Skills collection (9 skills)** — `prisma/skills` (MIT): Prisma ORM v7 breaking migration guide, mandatory SQL driver adapters, type-safe Prisma Client query API, CLI commands, serverless Prisma Postgres, containerized Prisma Compute deployment, and strategic terminal v6 guidance for MongoDB. Includes interactive `PrismaV7Lab` (`/prisma#v7-lab`).
- **Official TanStack Skills collection (14 skills)** — `tanstack-skills/tanstack-skills` & TanStack Intent (MIT): full suite across TanStack Router, TanStack Query, Table, Form, Start, Virtual, Store, AI, DB, Devtools, CLI, Config, Pacer, and Ranger, with deep type-safety, headless UI, and npm-bundled skills. Includes interactive `TanStackLab` (`/tanstack#reactive-lab`).
...
- **Interactive Provider Labs & Mental Models** — Dedicated interactive learning features per provider:
  - **Vercel**: Performance & Waterfall Lab (`/vercel#waterfall-lab`) simulating sequential waterfall latency vs `Promise.all` vs deferred branch fetch, boolean prop state explosion (2^N) vs compound components, and the 5-stage 14-day `signals.json` optimization gate; plus visual concepts for impact hierarchy, barrel imports, and React 19 composition.
  - **Anthropic**: Evaluation & Context Arena (`/anthropic#eval-arena`) simulating 3-tier progressive disclosure context window savings (88% token cut), `skill-creator` baseline-vs-skill eval benchmarks (`evals.json`), and deterministic Python scripts vs LLM XML corruption; plus visual concepts for 3-tier anatomy and feedback loops.
  - **Cloudflare**: Runtime & Binding Matrix (`/cloudflare#edge-matrix`) with an interactive primitive decision engine (DO vs Worker vs Agents SDK vs Sandbox @next), direct in-memory binding vs external REST latency benchmark (1.4ms vs 118ms), and stable vs @next Sandbox package line dichotomy; plus visual concepts for edge compute hierarchy and retrieval-over-pre-training.
