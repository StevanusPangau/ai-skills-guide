...
const skillIndexByName = new Map(
  superpowersSkills.map((skill, index) => [skill.name, index] as const),
)
...
export const Route = createFileRoute('/superpowers/skills/$skillName')({
  loader: ({ params }): { skill: RichSkill; prev: RichSkill | null; next: RichSkill | null } => {
    ...
    return {
      skill: superpowersSkills[index],
      prev: index > 0 ? superpowersSkills[index - 1] : null,
      next:
        index < superpowersSkills.length - 1
          ? superpowersSkills[index + 1]
          : null,
    }
  },
  component: SuperpowersSkillPage,
  notFoundComponent: SuperpowersSkillNotFound,
})
      ...
      <div className="mx-auto flex max-w-6xl gap-10 px-6 py-10">
        <div className="min-w-0 max-w-3xl flex-1 space-y-8">
          <div className="text-sm text-muted-foreground">
            ...
            <Link to="/superpowers" className={crumbLinkClass}>
              Superpowers
            </Link>
            <span className="mx-2">/</span>
              ...
              <section id="pairs-well-with" className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {skill.pairsWellWith.map((pair: string) => (
                    <Link
                      key={pair}
                      to="/superpowers/skills/$skillName"
          ...
          >
            {isEn ? '← Back to Superpowers Catalog' : '← Kembali ke Katalog Superpowers'}
          </Link>
        </div>
...
function SuperpowersSkillNotFound() {
  const isEn = getLocale() === 'en'
  return (
    <main id="main-content" className="min-w-0 flex-1 pt-14">
      <div className="mx-auto max-w-3xl space-y-4 px-6 py-20 text-center">
        ...
        <Link to="/superpowers" className="text-primary hover:underline">
          {isEn ? 'Back to Superpowers Guide' : 'Kembali ke Panduan Superpowers'}
        </Link>
      </div>
    </main>
  )
