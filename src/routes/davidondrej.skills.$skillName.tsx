...
import { DavidSkillDetailBody } from '@/features/davidondrej/skill-detail'
import { getDavidSkillTocItems } from '@/features/davidondrej/skill-detail-toc'
import { resetSkillDetailScroll } from '@/lib/scroll-to-section'
import { useDocumentTitle } from '@/lib/use-document-title'
import { m } from '@/paraglide/messages.js'

const skillIndexByName = new Map(
  davidondrejSkills.map((skill, index) => [skill.name, index] as const),
)
...
export const Route = createFileRoute('/davidondrej/skills/$skillName')({
  loader: ({ params }) => {
    ...
    return {
      ...
      next:
        index < davidondrejSkills.length - 1
          ? davidondrejSkills[index + 1]
          : null,
    }
  },
...
function DavidSkillPage() {
  ...
  const tocItems = getDavidSkillTocItems(skill)
      ...
      <div className="mx-auto flex max-w-6xl gap-10 px-6 py-10">
        <div className="min-w-0 max-w-3xl flex-1 space-y-8">
          <div className="text-sm text-muted-foreground">
            ...
            <Link to="/davidondrej" className={crumbLinkClass}>
              {m.nav_david_ondrej()}
            </Link>
            <span className="mx-2">/</span>
          ...
          <div
            className={`border-l-4 pl-4 ${compatibilityBorder(skill.compatibility)}`}
          >
          ...
          <DavidSkillDetailBody skill={skill} />
    ...
    <main id="main-content" className="min-w-0 flex-1 pt-14">
      <div className="mx-auto max-w-3xl space-y-4 px-6 py-20 text-center">
        <h1 className="text-2xl font-bold text-balance">
          {m.david_detail_not_found_title()}
        </h1>
        <p className="text-sm text-muted-foreground">
          {m.david_detail_not_found_body()}
        </p>
        <Link
          to="/davidondrej"
          className="inline-flex items-center gap-1 rounded-sm text-sm text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50"
