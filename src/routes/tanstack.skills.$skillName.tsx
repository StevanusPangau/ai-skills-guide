...
import { tanstackSkills, type RichSkill } from '@/data/tanstack-skills'
...
const skillIndexByName = new Map(
  tanstackSkills.map((skill, index) => [skill.name, index] as const),
)
...
export const Route = createFileRoute('/tanstack/skills/$skillName')({
  loader: ({ params }): { skill: RichSkill; prev: RichSkill | null; next: RichSkill | null } => {
    ...
    return {
      skill: tanstackSkills[index],
      prev: index > 0 ? tanstackSkills[index - 1] : null,
      next:
        index < tanstackSkills.length - 1
          ? tanstackSkills[index + 1]
          : null,
    }
  },
  component: TanStackSkillPage,
  notFoundComponent: TanStackSkillNotFound,
})
...
function TanStackSkillNotFound() {
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 text-center">
      <h1 className="text-2xl font-bold">Skill Not Found</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        Skill yang Anda cari tidak ditemukan dalam koleksi TanStack.
      </p>
      <Link
        to="/tanstack"
