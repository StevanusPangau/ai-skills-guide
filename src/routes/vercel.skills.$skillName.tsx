...
import { vercelSkills, type RichSkill } from '@/data/vercel-skills'
...
const skillIndexByName = new Map(
  vercelSkills.map((skill, index) => [skill.name, index] as const),
)
...
export const Route = createFileRoute('/vercel/skills/$skillName')({
  loader: ({ params }): { skill: RichSkill; prev: RichSkill | null; next: RichSkill | null } => {
    ...
    return {
      skill: vercelSkills[index],
      prev: index > 0 ? vercelSkills[index - 1] : null,
      next:
        index < vercelSkills.length - 1
          ? vercelSkills[index + 1]
          : null,
    }
  },
  component: VercelSkillPage,
  notFoundComponent: VercelSkillNotFound,
})
      ...
      <div className="mx-auto flex max-w-6xl gap-10 px-6 py-10">
        <div className="min-w-0 max-w-3xl flex-1 space-y-8">
          <div className="text-sm text-muted-foreground">
            ...
            <Link to="/vercel" className={crumbLinkClass}>
              Vercel
            </Link>
            <span className="mx-2">/</span>
          ...
          <div className="space-y-6">
            ...
            <div id="install" className="scroll-mt-20 space-y-3">
              ...
              <SkillInstallBlock source="vercel-labs/agent-skills" skillName={skill.name} />
            </div>
          </div>
          ...
          >
            {isEn ? '← Back to Vercel Catalog' : '← Kembali ke Katalog Vercel'}
          </Link>
        </div>
  ...
  return (
    <main id="main-content" className="min-w-0 flex-1 pt-14">
      <div className="mx-auto max-w-3xl space-y-4 px-6 py-20 text-center">
        ...
        <Link to="/vercel" className="text-primary hover:underline">
          {isEn ? 'Back to Vercel Guide' : 'Kembali ke Panduan Vercel'}
        </Link>
      </div>
    </main>
  )
