...
import { gsapSkills, type RichSkill } from '@/data/gsap-skills'
...
const skillIndexByName = new Map(
  gsapSkills.map((skill, index) => [skill.name, index] as const),
)
...
export const Route = createFileRoute('/gsap/skills/$skillName')({
  loader: ({ params }): { skill: RichSkill; prev: RichSkill | null; next: RichSkill | null } => {
    ...
    return {
      skill: gsapSkills[index],
      prev: index > 0 ? gsapSkills[index - 1] : null,
      next:
        index < gsapSkills.length - 1
          ? gsapSkills[index + 1]
          : null,
    }
  },
  component: GsapSkillPage,
  notFoundComponent: GsapSkillNotFound,
})
        ...
        <article className="min-w-0">
          <header className="space-y-4">
            ...
            >
              ...
              <span>/</span>
              <Link to="/gsap" className={crumbLinkClass}>
                GSAP
              </Link>
              <span>/</span>
          ...
          <div className="prose-clean space-y-8">
            ...
            <section id="install" className="scroll-mt-20 space-y-3">
              ...
              <SkillInstallBlock
                skillName={skill.name}
                source={author?.source ?? 'greensock/gsap-skills'}
              />
            </section>
          </div>
...
function GsapSkillNotFound() {
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 text-center">
      <h1 className="text-2xl font-bold">Skill Not Found</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        Skill yang Anda cari tidak ditemukan dalam koleksi GSAP.
      </p>
      <Link
        to="/gsap"
