...
import { expoSkills, type RichSkill } from '@/data/expo-skills'
...
const skillIndexByName = new Map(
  expoSkills.map((skill, index) => [skill.name, index] as const),
)
...
export const Route = createFileRoute('/expo/skills/$skillName')({
  loader: ({ params }): { skill: RichSkill; prev: RichSkill | null; next: RichSkill | null } => {
    ...
    return {
      skill: expoSkills[index],
      prev: index > 0 ? expoSkills[index - 1] : null,
      next:
        index < expoSkills.length - 1
          ? expoSkills[index + 1]
          : null,
    }
  },
  component: ExpoSkillPage,
  notFoundComponent: ExpoSkillNotFound,
})
        ...
        <article className="min-w-0">
          <header className="space-y-4">
            ...
            >
              ...
              <span>/</span>
              <Link to="/expo" className={crumbLinkClass}>
                Expo
              </Link>
              <span>/</span>
          ...
          <div className="prose-clean space-y-8">
            ...
            <section id="install" className="scroll-mt-20 space-y-3">
              ...
              <SkillInstallBlock
                skillName={skill.name}
                source={author?.source ?? 'expo/skills'}
              />
            </section>
          </div>
...
function ExpoSkillNotFound() {
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 text-center">
      <h1 className="text-2xl font-bold">Skill Not Found</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        Skill yang Anda cari tidak ditemukan dalam koleksi Expo.
      </p>
      <Link
        to="/expo"
