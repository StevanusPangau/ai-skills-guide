...
import { jakubkrehelSkills, type RichSkill } from '@/data/jakubkrehel-skills'
...
const skillIndexByName = new Map(
  jakubkrehelSkills.map((skill, index) => [skill.name, index] as const),
)
...
export const Route = createFileRoute('/jakubkrehel/skills/$skillName')({
  loader: ({ params }): { skill: RichSkill; prev: RichSkill | null; next: RichSkill | null } => {
    ...
    return {
      skill: jakubkrehelSkills[index],
      prev: index > 0 ? jakubkrehelSkills[index - 1] : null,
      next:
        index < jakubkrehelSkills.length - 1
          ? jakubkrehelSkills[index + 1]
          : null,
    }
  },
  component: JakubSkillPage,
  notFoundComponent: JakubSkillNotFound,
})
      ...
      <div className="mx-auto flex max-w-6xl gap-10 px-6 py-10">
        <div className="min-w-0 max-w-3xl flex-1 space-y-8">
          <div className="text-sm text-muted-foreground">
            ...
            <Link to="/jakubkrehel" className={crumbLinkClass}>
              Jakub Krehel
            </Link>
            <span className="mx-2">/</span>
          ...
          <div className="space-y-6">
              ...
              <section id="pairs-well-with" className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {skill.pairsWellWith.map((pair: string) => (
                    <Link
                      key={pair}
                      to="/jakubkrehel/skills/$skillName"
            ...
            <div id="install" className="scroll-mt-20">
              <SkillInstallBlock source="jakubkrehel/skills" skillName={skill.name} />
            </div>
          </div>
          ...
          >
            {isEn ? '← Back to Jakub Krehel Catalog' : '← Kembali ke Katalog Jakub Krehel'}
          </Link>
        </div>
  ...
  return (
    <main id="main-content" className="min-w-0 flex-1 pt-14">
      <div className="mx-auto max-w-3xl space-y-4 px-6 py-20 text-center">
        ...
        <Link to="/jakubkrehel" className="text-primary hover:underline">
          {isEn ? 'Back to Jakub Krehel Guide' : 'Kembali ke Panduan Jakub Krehel'}
        </Link>
      </div>
    </main>
  )
