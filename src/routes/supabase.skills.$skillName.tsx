...
import { supabaseSkills, type RichSkill } from '@/data/supabase-skills'
...
const skillIndexByName = new Map(
  supabaseSkills.map((skill, index) => [skill.name, index] as const),
)
...
export const Route = createFileRoute('/supabase/skills/$skillName')({
  loader: ({ params }): { skill: RichSkill; prev: RichSkill | null; next: RichSkill | null } => {
    ...
    return {
      skill: supabaseSkills[index],
      prev: index > 0 ? supabaseSkills[index - 1] : null,
      next:
        index < supabaseSkills.length - 1
          ? supabaseSkills[index + 1]
          : null,
    }
  },
  component: SupabaseSkillPage,
  notFoundComponent: SupabaseSkillNotFound,
})
        ...
        <article className="min-w-0">
          <header className="space-y-4">
            ...
            >
              ...
              <span>/</span>
              <Link to="/supabase" className={crumbLinkClass}>
                Supabase
              </Link>
              <span>/</span>
...
function SupabaseSkillNotFound() {
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 text-center">
      <h1 className="text-2xl font-bold">Skill Not Found</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        Skill yang Anda cari tidak ditemukan dalam koleksi Supabase.
      </p>
      <Link
        to="/supabase"
