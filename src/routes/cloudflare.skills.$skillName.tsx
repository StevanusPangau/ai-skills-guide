...
const skillIndexByName = new Map(
  cloudflareSkills.map((skill, index) => [skill.name, index] as const),
)
...
export const Route = createFileRoute('/cloudflare/skills/$skillName')({
  loader: ({ params }): { skill: RichSkill; prev: RichSkill | null; next: RichSkill | null } => {
    ...
    return {
      skill: cloudflareSkills[index],
      prev: index > 0 ? cloudflareSkills[index - 1] : null,
      next:
        index < cloudflareSkills.length - 1
          ? cloudflareSkills[index + 1]
          : null,
    }
  },
  component: CloudflareSkillPage,
  notFoundComponent: CloudflareSkillNotFound,
})
      ...
      <div className="mx-auto flex max-w-6xl gap-10 px-6 py-10">
        <div className="min-w-0 max-w-3xl flex-1 space-y-8">
          <div className="text-sm text-muted-foreground">
            ...
            <Link to="/cloudflare" className={crumbLinkClass}>
              Cloudflare
            </Link>
            <span className="mx-2">/</span>
              ...
              <section id="pairs-well-with" className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {skill.pairsWellWith.map((pair: string) => (
                    <Link
                      key={pair}
                      to="/cloudflare/skills/$skillName"
          ...
          >
            {isEn ? '← Back to Cloudflare Catalog' : '← Kembali ke Katalog Cloudflare'}
          </Link>
        </div>
...
function CloudflareSkillNotFound() {
  const isEn = getLocale() === 'en'
  return (
    <main id="main-content" className="min-w-0 flex-1 pt-14">
      <div className="mx-auto max-w-3xl space-y-4 px-6 py-20 text-center">
        ...
        <Link to="/cloudflare" className="text-primary hover:underline">
          {isEn ? 'Back to Cloudflare Guide' : 'Kembali ke Panduan Cloudflare'}
        </Link>
      </div>
    </main>
  )
