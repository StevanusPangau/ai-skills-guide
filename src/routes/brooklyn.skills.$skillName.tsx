...
import { brooklynSkills, type RichSkill } from '@/data/brooklyn-skills'
...
const skillIndexByName = new Map(
  brooklynSkills.map((skill, index) => [skill.name, index] as const),
)
...
export const Route = createFileRoute('/brooklyn/skills/$skillName')({
  loader: ({ params }): { skill: RichSkill; prev: RichSkill | null; next: RichSkill | null } => {
    ...
    return {
      skill: brooklynSkills[index],
      prev: index > 0 ? brooklynSkills[index - 1] : null,
      next:
        index < brooklynSkills.length - 1
          ? brooklynSkills[index + 1]
          : null,
    }
  },
  component: BrooklynSkillPage,
  notFoundComponent: BrooklynSkillNotFound,
})
      ...
      <div className="mx-auto flex max-w-6xl gap-10 px-6 py-10">
        <div className="min-w-0 max-w-3xl flex-1 space-y-8">
          <div className="text-sm text-muted-foreground">
            ...
            <Link to="/brooklyn" className={crumbLinkClass}>
              Brooklyn
            </Link>
            <span className="mx-2">/</span>
          ...
          <div className="space-y-6">
            ...
            <div id="install" className="scroll-mt-20 space-y-3">
              ...
              <SkillInstallBlock source="OutThisLife/brooklyn-skills" skillName={skill.name} />
            </div>
          </div>
          ...
          >
            {isEn ? '← Back to Brooklyn Catalog' : '← Kembali ke Katalog Brooklyn'}
          </Link>
        </div>
...
function BrooklynSkillNotFound() {
  const isEn = getLocale() === 'en'
  return (
    <main id="main-content" className="min-w-0 flex-1 pt-14">
      <div className="mx-auto max-w-3xl space-y-4 px-6 py-20 text-center">
        ...
        <Link to="/brooklyn" className="text-primary hover:underline">
          {isEn ? 'Back to Brooklyn Guide' : 'Kembali ke Panduan Brooklyn'}
        </Link>
      </div>
    </main>
  )
