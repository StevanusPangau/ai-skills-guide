...
export const Route = createFileRoute('/impeccable/skills/$skillName')({
  loader: ({ params }): { skill: RichSkill; prev: RichSkill | null; next: RichSkill | null } => {
    ...
    return {
      skill: impeccableSkills[index],
      prev: index > 0 ? impeccableSkills[index - 1] : null,
      next:
        index < impeccableSkills.length - 1
          ? impeccableSkills[index + 1]
          : null,
    }
  },
  component: ImpeccableSkillPage,
  notFoundComponent: ImpeccableSkillNotFound,
})
        ...
        <article className="min-w-0">
          <header className="space-y-4">
            ...
            >
              ...
              <span>/</span>
              <Link to="/impeccable" className={crumbLinkClass}>
                Impeccable
              </Link>
              <span>/</span>
...
function ImpeccableSkillNotFound() {
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 text-center">
      <h1 className="text-2xl font-bold">Skill Not Found</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        Skill yang Anda cari tidak ditemukan dalam koleksi Impeccable.
      </p>
      <Link
        to="/impeccable"
        className="mt-6 inline-block rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground"
      >
        Kembali ke Impeccable
      </Link>
    </div>
  )
}
