...
import { IMPECCABLE_SOURCE_REPO, impeccableSkills } from '@/data/impeccable-skills'
import { getCollectionBySlug } from '@/data/collections'
import { m } from '@/paraglide/messages.js'

export function ImpeccableOverview() {
    ...
    <section id="overview" className="scroll-mt-20 space-y-7">
      <div>
        ...
        >
          ...
          <span>github.com/pbakaus/impeccable</span>
        </a>
      </div>
      ...
      <Card className="border-2 border-primary/30 bg-primary/5">
        <CardHeader className="pb-2">
          <CardTitle as="h2" className="text-base">
            {m.impeccable_overview_question()}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm leading-relaxed text-muted-foreground">
            {m.impeccable_overview_answer()}
          </p>
        </CardContent>
      </Card>
      <p className="text-xs text-muted-foreground">{m.impeccable_footer()}</p>
    </section>
  )
}
